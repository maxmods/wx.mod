...
int wxSheetArrayEdge::FindIndex(int coord, bool clipToMinMax) const
{
    ...
    if (count == 0)
        return (i_max < int(m_count)) ? i_max : (clipToMinMax ? int(m_count)-1 : -1);
    ...
    // for "hidden" elements move to first one
    while ((i_min > 0) && (m_data[i_min] == m_data[i_min-1]))
        i_min--;
...
int wxSheetArrayEdge::FindMaxEdgeIndex(int val, int edge_size) const
{
    ...
    const int diff_min = (index > 0) ? abs(GetMin(index) - val) : diff+edge_size+1;
    ...
    else if (min_diff == diff_min)
    {
        ...
        {
            // for "hidden" elements move to first one
            while ((index > 0) && (m_data[index] == m_data[index-1]))
                index--;
        }
...
{
    wxCHECK_MSG(item < m_count, item*m_default_size, _T("invalid index") );
    return m_data.IsEmpty() ? item*m_default_size : (item > 0 ? m_data[item-1] : 0);
}
int wxSheetArrayEdge::GetMax(size_t item) const
{
    wxCHECK_MSG(item < m_count, (item+1)*m_default_size - 1, _T("invalid index") );
    return (m_data.IsEmpty() ? (item+1)*m_default_size : m_data[item]) - 1;
}
int wxSheetArrayEdge::GetSize(size_t item) const
{
    wxCHECK_MSG(item < m_count, m_default_size, _T("invalid index") );
    return m_data.IsEmpty() ? m_default_size : (m_data[item] - (item > 0 ? m_data[item-1] : 0));
}

void wxSheetArrayEdge::SetSize(size_t item, int size, bool ignore_min_size)
{
    wxCHECK_RET(item < m_count, wxT("Invalid edge index"));
    wxCHECK_RET(size >= 0, wxT("Invalid edge size")); // this would break things
...
{
    const int min_index = m_minSizes.Index(index);
    return (min_index != wxNOT_FOUND) ? m_minSizes[min_index] :
                                        m_min_allowed_size;
}
void wxSheetArrayEdge::SetMinSize(size_t index, int size)
{
    if (size > m_min_allowed_size)
        m_minSizes.Add(index, size);
    else
        m_minSizes.Remove(index);
}
...
bool wxSheetArrayEdge::SetMinAllowedSize(int min_allowed_size, bool resizeExisting)
{
    ...
    {
        int n, count = m_minSizes.GetCount();
        for (n = 0; n < count; n++)
        {
            if (m_minSizes[n] < min_allowed_size)
            {
                m_minSizes.RemoveAt(n);
                n--;
                count--;
            }
        }
