...
// Created:     11/04/2001
// RCS-ID:      $Id: sheetren.cpp,v 1.5 2007/12/12 05:22:39 jrl1 Exp $
                                                     ...
                                                     const wxSheetCoords& coords)
{
    ...
    int margin = MARGIN; // FIXME - make adjustable perhaps

    if (((m_align & wxSHEET_BMPREN_BMPABOVE) != 0) ||
        ((m_align & wxSHEET_BMPREN_BMPBELOW) != 0))
        return wxSize(wxMax(bmpSize.x, strSize.x) + 2*margin, bmpSize.y + strSize.y + 3*margin);

    // else to left or right
    return wxSize(bmpSize.x + strSize.x + 3*margin, wxMax(bmpSize.y, strSize.y) + 2*margin);
}
