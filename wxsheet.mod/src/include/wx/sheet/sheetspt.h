...
enum wxSheetSplitMode_Type
{
    ...
    wxSHEET_SPLIT_VERTICAL,   // split vertically   (splitter line vertical)
    wxSHEET_SPLIT_HORIZONTAL  // split horizontally (splitter line horizontal)
};

// These window styles may be used from wxSplitterWindow
...
public:
    ...
    virtual void UnsplitVertically(bool remove_right = true, bool sendEvt = false);
    virtual void UnsplitHorizontally(bool remove_bottom = true, bool sendEvt = false);

    // Get the position of the sash, 0 if not split
    int GetVerticalSplitPosition() const   { return m_splitPos.x; }
    int GetHorizontalSplitPosition() const { return m_splitPos.y; }
    ...
    virtual void SetVerticalSplitPosition(int x_pos, bool sendEvt = false);
    virtual void SetHorizontalSplitPosition(int y_pos, bool sendEvt = false);

    // The top left window is the window you Initialized this with, always valid
...
class WXDLLIMPEXP_SHEET wxSheetSplitterEvent : public wxNotifyEvent
{
public:
    ...
    bool IsVerticalSplit() const { return m_split_mode == wxSHEET_SPLIT_VERTICAL; }
...
#define wxSheetSplitterEventHandler(func) \
    (wxObjectEventFunction)(wxEventFunction)wxStaticCastEvent(wxSheetSplitterEventFunction, &func)

#define wx__DECLARE_SHEETSPLITEVT(evt, id, fn)  wx__DECLARE_EVT1( evt, id, wxSheetSplitterEventHandler(fn))

#define EVT_SHEET_SPLIT_BEGIN(id, fn)         wx__DECLARE_SHEETSPLITEVT( wxEVT_SHEET_SPLIT_BEGIN,         id, fn )
#define EVT_SHEET_SPLIT_CHANGING(id, fn)      wx__DECLARE_SHEETSPLITEVT( wxEVT_SHEET_SPLIT_CHANGING,      id, fn )
