...
bool wxSheetCellAttr::MergeWith(const wxSheetCellAttr &other)
{
    ...
    if ( !HasForegroundColour() && other.HasForegroundColour() )
        SetForegroundColour(other.GetForegroundColour());
...
bool wxSheetCellAttr::HasForegroundColour() const
{
...
{
    return Ok() && HasForegroundColour() && HasBackgroundColour() && HasFont() &&
...
const wxColour& wxSheetCellAttr::GetForegroundColour() const
{
    ...
    else if (GetDefaultAttr().Ok()) // && GetDefaultAttr().HasForegroundColour())
...
wxSheetCellRenderer wxSheetCellAttr::GetRenderer(wxSheet* sheet, const wxSheetCoords& coords) const
{
    ...
    wxCHECK_MSG(renderer.Ok(), renderer, wxT("Missing default cell renderer"));
    return renderer;
}
wxSheetCellEditor wxSheetCellAttr::GetEditor(wxSheet* sheet, const wxSheetCoords& coords) const
{
    ...
    // we're supposed to always find something
    wxCHECK_MSG(editor.Ok(), editor, wxT("Missing default cell editor"));
    return editor;
}
...
// ----------------------------------------------------------------------------
IMPLEMENT_ABSTRACT_CLASS(wxSheetCellAttrProvider, wxObject)

void wxSheetCellAttrProvider::Copy( const wxSheetCellAttrProvider& other )
{
    m_cellAttrs     = other.m_cellAttrs;
    m_rowAttrs      = other.m_rowAttrs;
    m_colAttrs      = other.m_colAttrs;
    m_rowLabelAttrs = other.m_rowLabelAttrs;
    m_colLabelAttrs = other.m_colLabelAttrs;
}

void wxSheetCellAttrProvider::Clear( int update )
{
    ...
    if ((update & wxSHEET_UpdateRowLabelAttrs) != 0)
        m_rowLabelAttrs.Clear();
    if ((update & wxSHEET_UpdateColLabelAttrs) != 0)
        m_colLabelAttrs.Clear();
}
        ...
        case wxSHEET_CELL_GRID :
        {
            switch (type)
            {
                case wxSHEET_AttrAny:
                {
                    ...
                    if (attrCol.Ok())  data.Add(attrCol.GetLevel()+2, attrCol);
                    if (attrRow.Ok())  data.Add(attrRow.GetLevel()+1, attrRow);
                    if (attrCell.Ok()) data.Add(attrCell.GetLevel()+0, attrCell);
                    ...
                    switch (count)
                    {
                        ...
                        case 1  : return data.ItemValue(0);
                        default : break;
                    }
                    ...
                    attr.Copy(data.ItemValue(0));
                    attr.SetDefaultAttr(data.ItemValue(1));
                    if (count > 2)
                        data.ItemValue(1).SetDefaultAttr(data.ItemValue(2));
/*
                    // This is the old way of copying the values to a single attr
                    wxSheetCellAttr attr;
                    attr.Copy(data.ItemValue(0));
                    attr.MergeWith(data.ItemValue(1));
                    if (count > 2)
                       attr.MergeWith(data.ItemValue(2));
*/
    ...
    switch (wxSheet::GetCellCoordsType(coords))
    {
        case wxSHEET_CELL_GRID :
        {
            switch (type)
            {
                ...
                {
                    if (attr.Ok()) m_cellAttrs.Add(coords, attr);
                    else           m_cellAttrs.Remove(coords);
                    return;
                }
                case wxSHEET_AttrRow :
                {
                    if (attr.Ok()) m_rowAttrs.Add(coords.m_row, attr);
                    else           m_rowAttrs.Remove(coords.m_row);
                    return;
                }
                case wxSHEET_AttrCol :
                {
                    if (attr.Ok()) m_colAttrs.Add(coords.m_col, attr);
                    else           m_colAttrs.Remove(coords.m_col);
                    return;
                }
                default :
                {
        ...
        case wxSHEET_CELL_ROWLABEL :
        {
            switch (type)
            {
                case wxSHEET_AttrCell :
                {
                    if (attr.Ok()) m_rowLabelAttrs.Add(coords.m_row, attr);
                    else           m_rowLabelAttrs.Remove(coords.m_row);
                    return;
                }
                default :
                {
        ...
        case wxSHEET_CELL_COLLABEL :
        {
            switch (type)
            {
                case wxSHEET_AttrCell :
                {
                    if (attr.Ok()) m_colLabelAttrs.Add(coords.m_col, attr);
                    else           m_colLabelAttrs.Remove(coords.m_col);
                    return;
                }
                default :
                {
