...
// wxSheet code is loosely on the wxGrid, to ensure that it stays up to date
// changes in the wxGrid files should be noted and incorporated if applicable.
// Note: very little code remains, update colours, sizing, unicode...
// wx/include/wx/generic/grid.h         1.147
// wx/include/wx/generic/gridctrl.h     1.15
// wx/include/wx/generic/gridsel.h      1.18
// wx/src/generic/grid.cpp              1.338
// wx/src/generic/gridctrl.cpp          1.16
// wx/src/generic/gridsel.cpp           1.21
...
{
    // FIXME probably a bug in wxWidgets, but they hacked wxGrid to do the same
    if (event.ButtonDown(wxMOUSE_BTN_ANY) && (FindFocus() != this))
        SetFocus();
...
wxSheetRefData::wxSheetRefData()
{
    ...
    m_scrollThumbSize  = wxSize(15, 15);
    m_equal_col_widths = 0;

    m_gridLineColour   = wxColour( 192, 192, 192 );
    m_gridLinesEnabled = wxBOTH;

    m_cursorCellHighlightPen   = wxPen(*wxBLACK, 2, wxSOLID);
    m_cursorCellHighlightROPen = wxPen(*wxBLACK, 2, wxSHORT_DASH);
    ...
    m_batchCount = 0;
    m_editable   = wxSHEET_CELL_ANY;  // default for whole grid
...
void wxSheetRefData::RemoveSheet(wxSheet* sheet)
{
    ...
    // not an error if not found, allows for this to be called to stop event
    // processing when being destroyed and, if that wasn't called, again in destructor.
                      ...
                      long style, const wxString& name )
{
    ...
    // double check with the height of a combobox using the same font
    static int s_combo_height = -1;
    if (s_combo_height == -1)
    {
        wxComboBox combo(this, -1, wxT("W1!(jJ"));
        combo.SetFont(labelFont);
        s_combo_height = combo.GetBestSize().y;
        s_combo_height = wxMax(s_combo_height, char_h);
    }

    GetSheetRefData()->m_rowEdges.SetDefaultSize(char_h);
    GetSheetRefData()->m_colLabelHeight = s_combo_height;
...
bool wxSheet::SetTable( wxSheetTable *table, bool takeOwnership )
{
    ...
    if (GetNumberRows() != 0)
        DoUpdateRows(0, -GetNumberRows());
    if (GetNumberCols() != 0)
        DoUpdateCols(0, -GetNumberCols());
    ...
    if (table)
    {
        if (table->GetNumberRows() != 0)
            DoUpdateRows(0, table->GetNumberRows());
        if (table->GetNumberCols() != 0)
            DoUpdateCols(0, table->GetNumberCols());
...
bool wxSheet::CreateGrid( int numRows, int numCols, int options )
{
    ...
    table->SetGridCellValueProvider(new wxSheetValueProviderString(0, 0, options), true);
    ...
    table->SetRowLabelValueProvider(new wxSheetValueProviderString(0, 1), true);
    table->SetColLabelValueProvider(new wxSheetValueProviderString(1, 0, wxSHEET_ValueProviderColPref), true);
    ...
    // now update all the containers to have the proper number of rows/cols
    SetNumberCells(numRows, numCols);
...
{
    wxSHEET_CHECKUPDATE_MSG(row, numRows, GetNumberRows(), false)
    ...
    // Update the cursor, trying to keep it valid
    wxSheetCoords cursorCoords(GetSheetRefData()->m_cursorCoords);
    cursorCoords.UpdateRows(row, numRows);
    // try to force the cursor to a valid cell
    if (!ContainsGridRow(cursorCoords.m_row))
        cursorCoords.m_row = 0;

    GetSheetRefData()->m_cursorCoords = cursorCoords;

    SetGridOrigin(-1, -1, true);
...
{
    wxSHEET_CHECKUPDATE_MSG(col, numCols, GetNumberCols(), false)
    ...
    wxSheetCoords cursorCoords(GetSheetRefData()->m_cursorCoords);
    cursorCoords.UpdateCols(col, numCols);
    if (!ContainsGridRow(cursorCoords.m_col))
        cursorCoords.m_col = 0;
...
void wxSheet::SetRowHeight( int row, int height, bool ignore_min_size )
{
    if (row == -1)
    {
    ...
    GetSheetRefData()->m_rowEdges.SetSize(row, height, ignore_min_size);
...
void wxSheet::SetColWidth( int col, int width, bool ignore_min_size )
{
    if (col == -1)
    {
    ...
    GetSheetRefData()->m_colEdges.SetSize(col, width, ignore_min_size);
...
{
    wxCHECK_MSG((row == -1) || ContainsGridRow(row), GetDefaultRowHeight(), wxT("Invalid coords in wxSheet::GetRowBestHeight"));
...
{
    wxCHECK_MSG((col == -1) || ContainsGridCol(col), GetDefaultColWidth(), wxT("Invalid col in wxSheet::GetColBestWidth"));
    wxClientDC dc(m_gridWin);
    wxCoord bestWidth = 0;
...
{
    wxCHECK_MSG((row == -1) || ContainsGridRow(row), 0, wxT("Invalid coords in wxSheet::AutoSizeRow"));
...
{
    wxCHECK_MSG((col == -1) || ContainsGridCol(col), 0, wxT("Invalid col in wxSheet::AutoSizeCol"));
...
void wxSheet::AutoSize(bool setAsMin)
{
    ...
    wxSize sizeFit(GetSheetVirtualSize());
...
void wxSheet::SetCursorCellHighlightPen(const wxPen& pen)
{
    GetSheetRefData()->m_cursorCellHighlightPen = pen;

    if (ContainsCell(GetGridCursorCell()))
        RefreshCell(GetGridCursorCell(), true);
}

void wxSheet::SetCursorCellHighlightROPen(const wxPen& pen)
{
    GetSheetRefData()->m_cursorCellHighlightROPen = pen;
...
const wxColour& wxSheet::GetAttrForegroundColour( const wxSheetCoords& coords, wxSheetAttr_Type type ) const
{
...
void wxSheet::MakeCellVisible( const wxSheetCoords& coords )
{
    ...
    if (!IsColLabelCell(coords))
    {
        ...
        {
            ypos = logRect.GetTop();
            too_tall = devRect.height > ch - SHEET_SCROLL_LINE_Y;
            ypos -= too_tall ? 1 : SHEET_SCROLL_LINE_Y; // add a little padding
            if (ypos < 0) ypos = 0;
        }
        else if ( !too_tall && (devRect.GetBottom() > ch) )
        {
    ...
    if (!IsRowLabelCell(coords))
    {
        if ( (too_wide && (devRect.x > 1)) || (devRect.GetLeft() < 0) )
        {
            ...
            too_wide = devRect.width  > cw - SHEET_SCROLL_LINE_X;
            xpos -= too_wide ? 1 : SHEET_SCROLL_LINE_X;
            if (xpos < 0) xpos = 0;
        }
        else if ( !too_wide && (devRect.GetRight() > cw) )
        {
...
{
    return IsBlockSelected(wxSheetBlock(coords, 1, 1));
}
bool wxSheet::IsRowSelected( int row ) const
{
    return IsBlockSelected(wxSheetBlock(row, 0, 1, GetNumberCols()));
}
bool wxSheet::IsColSelected( int col ) const
{
    return IsBlockSelected( wxSheetBlock(0, col, GetNumberRows(), 1) );
}
bool wxSheet::IsBlockSelected( const wxSheetBlock& block ) const
{
    // if only one cell, this is faster
    if (block.IsOneCell())
    {
        wxSheetCoords coords(block.GetLeftTop());
        return ( GetSelectingBlock().Contains(coords) ||
                (GetSelection() && GetSelection()->Contains(coords)) );
    }

    return ( GetSelectingBlock().Contains(block) ||
            (GetSelection() && GetSelection()->Contains(block)) );
}
...
void wxSheet::SetSelectionMode(wxSheetSelectionMode_Type selmode)
{
    ...
    // You'd have to send a slew of confusing (de)selection events
    ClearSelection(true);
    GetSheetRefData()->m_selectionMode = selmode;
}

wxSheetBlock wxSheet::FixSelectionBlock(const wxSheetBlock& block_,
                                        int sel_mode) const
{
    wxSheetBlock block(block_.GetAligned());

    if (sel_mode == -1) sel_mode = GetSelectionMode();

    // make sure block that's selected goes full width/height
    if ((sel_mode == wxSHEET_SelectRows) || ((sel_mode & wxSHEET_SelectRows) != 0))
    {
        block.SetLeft(0);
        block.SetWidth(GetNumberCols() + 1);
    }
    else if ((sel_mode == wxSHEET_SelectCols) || ((sel_mode & wxSHEET_SelectCols) != 0))
    {
        block.SetTop(0);
...
bool wxSheet::SelectBlock( const wxSheetBlock& block, bool addToSelected, bool sendEvt )
{
    ...
    if (sendEvt)
        SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTED, block, true, addToSelected);
...
bool wxSheet::DeselectRow( int row, bool sendEvt )
{
    ...
    return DeselectBlock( wxSheetBlock(row, -1, 1, GetNumberCols()+1), sendEvt );
}
...
bool wxSheet::DeselectBlock( const wxSheetBlock& block, bool sendEvt )
{
    ...
    if (sendEvt)
        SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTED, block, false, false);
...
bool wxSheet::ClearSelection(bool send_event)
{
    ...
    if (send_event)
    {
        ...
        SendRangeSelectEvent( wxEVT_SHEET_RANGE_SELECTED,
...
void wxSheetDataObject::CreateDataString()
{
    ...
    int row, col;
    int last_row = values.ItemKey(0) - 10, last_col;

    for (r = 0; r < nrows; r++)
    {
        row = values.ItemKey(r);
        if (last_row+1 < row)
            m_data += s_newRow + wxString::Format(wxT("%d:"), row);
        else
            m_data += s_nextRow;

        col = values.ItemValue(r).ItemKey(0);
        m_data += s_newCol + wxString::Format(wxT("%d:"), col);
        last_col = col;

        ncols = values.ItemValue(r).GetCount();
        for (c = 0; c < ncols; c++)
        {
            col = values.ItemValue(r).ItemKey(c);
            if (last_col+1 < col)
                m_data += s_newCol + wxString::Format(wxT("%d:"), col);

            m_data += values.ItemValue(r).ItemValue(c) + s_nextCol;
            last_col = col;
        }
    }
}
...
bool wxSheetDataObject::SetData(size_t len, const void *buf)
{
    ...
    for ( n = 0; n < str_len-1; n++, c++ )
    {
        ...
        {
            wxString strVal(strBuf.Mid(last_n, n-last_n));
            values.GetOrCreateValue(row).Add(col, strVal);
...
{
    CopyCurrentSelectionToInternalSelection(copy_cursor);
                                  ...
                                  const wxChar& colSep )
{
    ...
    else if (wxTheClipboard->IsSupported(wxDF_TEXT))
    {
        ...
        {
            CopyStringToInternalSelection(textData.GetText(), colSep);
            ret = PasteInternalCopiedSelection( topLeft );
        }
    }
...
bool wxSheet::CopyCurrentSelectionToInternalSelection(bool copy_cursor)
{
    wxPairArrayIntPairArraySheetString &copiedData = GetSheetRefData()->m_copiedData.GetData();
    copiedData.Clear();

    wxSheetSelection sel;
    if (GetSelection())
        sel = *GetSelection();
    ...
    wxSheetCoords cell;
    while (selIter.GetNext(cell) != wxSHEET_SELITER_GET_END)
        copiedData.GetOrCreateValue(cell.m_row).Add(cell.m_col, GetCellValue(cell));
...
bool wxSheet::CopyStringToInternalSelection(const wxString &string, const wxChar& colSep)
{
    wxPairArrayIntPairArraySheetString &copiedData = GetSheetRefData()->m_copiedData.GetData();
    copiedData.Clear();
    ...
    for (n=0; n<len; n++, c++)
    {
        if (((*c) == wxT('\r')) || ((*c) == wxT('\n')))
        {
            ...
            copiedData.GetOrCreateValue(row).Add(col, buf);
            col = 0;
            row++;
            buf.Empty();
        }
...
bool wxSheet::PasteInternalCopiedSelection(const wxSheetCoords &topLeft_)
{
    ...
    // find the full extent of the selected area and what topLeft cell to use
    wxSheetBlock currentBlock;
    if (GetSelection())
        currentBlock = GetSelection()->GetBoundingBlock();

    currentBlock.ExpandUnion(GetSelectingBlock());
    bool is_selection = !currentBlock.IsEmpty();

    PRINT_BLOCK("Current block ", currentBlock); fflush(stdout);
    ...
    // Find the top left cell that was copied
    wxSheetCoords copiedTopLeft( copiedData.ItemKey(0),
                                 copiedData.ItemValue(0).ItemKey(0) );
    ...
    // This is the full extent of the copied data block
    wxSheetBlock copiedBlock(copiedData.ItemKey(0),
                             copiedData.ItemValue(0).ItemKey(0),
        copiedData.ItemKey(copiedData.GetCount()-1) - copiedData.ItemKey(0) + 1,
        copiedData.ItemValue(0).ItemKey(copiedData.ItemValue(0).GetCount()-1) - copiedData.ItemValue(0).ItemKey(0) + 1);

    for (r = 1; r < copiedData.GetCount(); r++)
    {
        if (copiedBlock.GetLeft() > copiedData.ItemValue(r).ItemKey(0))
            copiedBlock.SetLeft(copiedData.ItemValue(r).ItemKey(0));

        int rcnt = copiedData.ItemValue(r).GetCount()-1;

        if (copiedBlock.GetRight() < copiedData.ItemValue(r).ItemKey(rcnt))
            copiedBlock.SetRight(copiedData.ItemValue(r).ItemKey(rcnt));
    }

    PRINT_BLOCK("Copied block ", copiedBlock); fflush(stdout);

    // paste into consecutive areas of the currentBlock with the copiedBlock
    int num_row_blocks = currentBlock.GetHeight()/copiedBlock.GetHeight();
    for (int row_blocks = 0; row_blocks <= num_row_blocks; row_blocks++)
    {
        wxSheetCoords curShiftCell = shiftCell;
        curShiftCell.m_row += row_blocks*copiedBlock.GetHeight();

        int num_col_blocks = currentBlock.GetWidth()/copiedBlock.GetWidth();
        for (int col_blocks = 0; col_blocks <= num_col_blocks; col_blocks++)
        {
            curShiftCell.m_col += col_blocks*copiedBlock.GetWidth();

            size_t n_copiedrows = copiedData.GetCount();
            for (r = 0; r < n_copiedrows; r++)
            {
                cell.m_row = copiedData.ItemKey(r);
                if (cell.m_row+curShiftCell.m_row >= GetNumberRows())
                    break;

                size_t n_copiedcols = copiedData.ItemValue(r).GetCount();

                for (c = 0; c < n_copiedcols; c++)
                {
                    cell.m_col = copiedData.ItemValue(r).ItemKey(c);
                    pasteCell = cell + curShiftCell;

                    if (pasteCell.m_col < 0) // FIXME should pasting non rect start w/ col from
                        continue;            //  first row or min col from all the rows
                    if (pasteCell.m_col >= GetNumberCols())
                        break;

                    const wxString& value = copiedData.ItemValue(r).ItemValue(c);

                    //wxPrintf(wxT("Pasting (%d %d), at (%d %d) '%s'\n"), r, c, pasteCell.m_row, pasteCell.m_col, value.c_str()); fflush(stdout);
                    //wxPrintf(wxT("      tl(%d %d), c  (%d %d) shift (%d %d)\n"), topLeft.m_row, topLeft.m_col, copiedTopLeft.m_row, copiedTopLeft.m_col, shiftCell.m_row, shiftCell.m_col); fflush(stdout);

                    // only paste into selected cells if selection
                    if (is_selection && !IsCellSelected(pasteCell))
                        continue;

                    if (SendEvent(wxEVT_SHEET_CELL_VALUE_CHANGING, pasteCell, NULL, value) != EVT_VETOED)
                    {
                        SetCellValue(pasteCell, value);
                        SendEvent(wxEVT_SHEET_CELL_VALUE_CHANGED, pasteCell, NULL, value);
                    }
                }
            }
...
void wxSheet::SetAreaEditable( int cell_type )
{
    // TODO: improve this ?
...
bool wxSheet::ShowCellEditControl()
{
    ...
    if (!GetEditControl().IsCreated())
    {
        ...
        SendEditorCreatedEvent(wxEVT_SHEET_EDITOR_CREATED, GetEditControlCoords(),
    ...
    GetSheetRefData()->m_cellEditor.InitEditor(GetEditControlCoords(), attr, this);
    ...
    if (attr.GetOverflow() && (GetEditControlCoords().m_col < GetNumberCols()))
    {
        wxSize bestSize = GetEditControl().GetBestSize(*this, attr, GetEditControlCoords());
        if (bestSize.x > rect.width)
        {
            ...
            for (c.m_col = c.m_col + cellSize.m_col; c.m_col < numCols; c.m_col++)
            {
               ...
               // looks weird going over a spanned cell
                if ((rect.width < bestSize.x) && (rect.GetRight() < clientWidth) && (cellSize == oneCell))
...
void wxSheet::SaveEditControlValue()
{
    ...
    {
        SendEvent(wxEVT_SHEET_CELL_VALUE_CHANGED, GetEditControlCoords());
...
{
    if ( GetSheetRefData()->m_batchCount > 0 )
    {
        GetSheetRefData()->m_batchCount--;
        if ( !GetSheetRefData()->m_batchCount && refresh )
        {
            GetSheetRefData()->m_batchCount++; // temp block refresh for window sizing
            AdjustScrollbars();
            GetSheetRefData()->m_batchCount--;
            Refresh(true);
        }
...
void wxSheet::RefreshCell(const wxSheetCoords& coords, bool single_cell)
{
    ...
    else if (ContainsGridCell(coords))
    {
        if (!single_cell)
        {
            ...
            rect.width = GetColRight(GetNumberCols()-1) - rect.x;
        }
...
void wxSheet::PaintSheetWindow( wxDC& dc, const wxRegion& WXUNUSED(reg) )
{
    ...
    if (m_vertScrollBar->IsShown() && m_horizScrollBar->IsShown())
    {
        ...
        int left = m_horizScrollBar->GetRect().GetRight() + m_vertSplitRect.GetWidth();
...
void wxSheet::PaintRowLabelWindow( wxDC& dc, const wxRegion& reg )
{
    ...
    // Now draw the dividing lines, can be skipped if refresh is only along line
    dc.SetPen(wxPen(GetLabelOutlineColour(), 1, wxSOLID));
    int top;
    CalcUnscrolledPosition(0, 0, NULL, &top);
    int bottom = top + m_gridWin->GetSize().y;
...
void wxSheet::PaintColLabelWindow( wxDC& dc, const wxRegion& reg )
{
    ...
    int right = left + m_gridWin->GetSize().x;
...
void wxSheet::DrawGridCells( wxDC& dc, const wxSheetSelection& blockSel )
{
    ...
    wxSheetSelectionIterator revIter(blockSel, wxSHEET_SELITER_REVERSE);
    while (revIter.GetNext(coords) != wxSHEET_SELITER_GET_END)
    {
        ...
        if ((lastCoords.m_row > coords.m_row) && !HasCellValue(lastCoords))
        {
            ...
            if ((overflow_col >= 0) && !blockSel.Contains(lastCoords.m_row, overflow_col))
                leftCols.Add(lastCoords.m_row, overflow_col);
        }
    ...
    for (n = 0; n < count; n++)
    {
        coords.Set(leftCols.ItemKey(n), leftCols.ItemValue(n));
        DrawCell( dc, coords );
...
void wxSheet::DrawGridSpace( wxDC& dc )
{
    ...
    if ( (right > rightCol) || (bottom > bottomRow) )
    {
        ...
        dc.SetBrush( wxBrush(m_gridWin->GetBackgroundColour(), wxSOLID) );
        dc.SetPen( *wxTRANSPARENT_PEN );
...
void wxSheet::DrawCursorCellHighlight( wxDC& dc, const wxSheetCellAttr& attr )
{
    ...
    wxPen pen = attr.GetReadOnly() ? GetCursorCellHighlightROPen() :
                                     GetCursorCellHighlightPen();
    ...
    if (pen.Ok())
    {
        ...
        if (IsCellSelected(coords)) pen.SetColour(GetSelectionForeground());
...
void wxSheet::DrawRowLabels( wxDC& dc, const wxArrayInt& rowLabels )
{
    size_t i, numLabels = rowLabels.GetCount();
    if ( !GetNumberRows() || !numLabels ) return;

    for ( i = 0; i < numLabels; i++ )
        DrawCell( dc, wxSheetCoords(rowLabels[i], -1) );
...
*/
    ...
    for ( i = 0; i < numLabels; i++ )
    {
        coords.m_row = rowLabels[i];
        bottom = GetRowBottom(GetCellBlock(coords).GetBottom());
        dc.DrawLine(0, bottom, width-1, bottom);
    }
}

void wxSheet::DrawColLabels( wxDC& dc, const wxArrayInt& colLabels )
{
    size_t i, numLabels = colLabels.GetCount();
    if ( !GetNumberCols() || !numLabels ) return;

    for ( i = 0; i < numLabels; i++ )
        DrawCell( dc, wxSheetCoords(-1, colLabels[i]) );
                                 ...
                                 int text_orientation )
{
    ...
    if (StringToLines( value, lines ) > 0)
        DrawTextRectangle( dc, lines, rect, align, text_orientation );
}
                                 ...
                                 int text_orientation )
{
    ...
    for ( l = 0; l < nLines; l++ )
    {
        dc.GetTextExtent(lines[l], &line_width, &line_height);
        lineWidths.Add(line_width);
    ...
    if ( text_orientation == wxVERTICAL )
    {
        long tmp = text_height;
    ...
    if ((align & wxALIGN_BOTTOM) != 0)
    {
        if ( text_orientation == wxHORIZONTAL )
            y = rect.y + (rect.height - text_height);
        else // wxVERTICAL
    ...
    {
        if ( text_orientation == wxHORIZONTAL )
            y = rect.y + (rect.height - text_height)/2.0;
        else // wxVERTICAL
    ...
    for ( l = 0; l < nLines; l++ )
    {
        line_width  = lineWidths[l];
        line_height = lineHeights[l];
        ...
        else if ((align & wxALIGN_CENTRE_HORIZONTAL) != 0)
        {
            if ( text_orientation == wxHORIZONTAL )
                ...
                y = rect.y + rect.height - (rect.height - line_width)/2.0;
        }
        else // wxALIGN_LEFT
        {
            if ( text_orientation == wxHORIZONTAL )
...
int wxSheet::YToEdgeOfGridRow( int y, int edge_size ) const
{
    return GetSheetRefData()->m_rowEdges.FindMaxEdgeIndex(y, edge_size);
}
int wxSheet::XToEdgeOfGridCol( int x, int edge_size ) const
{
    return GetSheetRefData()->m_colEdges.FindMaxEdgeIndex(x, edge_size);
}
...
wxPoint wxSheet::AlignInRect( int align, const wxRect& rect, const wxSize& size, bool inside )
{
...
wxSize wxSheet::GetSheetVirtualSize(bool add_margin) const
{
    return GetGridVirtualSize(add_margin) +
           wxSize(GetRowLabelWidth(), GetColLabelHeight());
}
...
{
    if (!m_gridWin || !m_horizScrollBar || !m_vertScrollBar)
        return;
    ...
    if ((x < 0) || (virtSize.x < cw))
        x = 0;
    else if (x > virtSize.x - cw + 1)
        x = virtSize.x - cw + 1;
    if ((y < 0) || (virtSize.y < ch))
        y = 0;
    else if (y > virtSize.y - ch + 1)
        y = virtSize.y - ch + 1;
...
#ifdef MSW_SCROLL_DOESNT_SCROLL_WIN_ONLY_RECT //__WXMSW__
    // GTK calcs rect for you, does MSW?
    ...
    m_gridWin->ScrollWindow( dx, dy, r );
    if (dx != 0)
        m_colLabelWin->ScrollWindow( dx, 0, r );
    if (dy != 0)
        m_rowLabelWin->ScrollWindow( 0, dy, r );
...
void wxSheet::OnScroll( wxScrollEvent &event )
{
    ...
    // this is a duplicate event and is not necessary
    if (event.GetEventType() == wxEVT_SCROLL_CHANGED)
        return;

    int pos = event.GetPosition();
    //wxPrintf(wxT("Pos %d %d Length %d Left %d Right %d\n"), pos, m_horizScrollBar->GetThumbPosition(), m_horizScrollBar->GetThumbSize(), pos*m_horizScrollBar->GetThumbSize(), pos*m_horizScrollBar->GetThumbSize()+GetGridWindow()->GetClientSize().x); fflush(stdout);
...
{
    if (!m_gridWin || !m_horizScrollBar || !m_vertScrollBar || m_resizing)
        return;
...
*/
    ...
    bool need_Xscroll1 = (m_scrollBarMode & SB_HORIZ_NEVER) != 0 ? false :
        (((m_scrollBarMode & SB_HORIZ_ALWAYS) != 0) ? true : virtSize.x > gw);
    bool need_Yscroll1 = (m_scrollBarMode & SB_VERT_NEVER ) != 0 ? false :
        (((m_scrollBarMode & SB_VERT_ALWAYS ) != 0) ? true : virtSize.y > gh);

    // Now cut down size due to the scrollbars if shown
    if (need_Xscroll1) gh -= sb_height;
    if (need_Yscroll1) gw -= sb_width;
    ...
    if (!need_Xscroll1 && need_Xscroll) gh -= sb_height;
    if (!need_Yscroll1 && need_Yscroll) gw -= sb_width;

    // set scrollbar parameters
    int thumbX = GetSheetRefData()->m_scrollThumbSize.x;
    int thumbY = GetSheetRefData()->m_scrollThumbSize.y;
    wxCHECK_RET((thumbX > 0) && (thumbY > 0), wxT("Invalid scrollbar thumb sizes"));

#if 0
    //This works for a scroll of 1
...
#else // this works in MSW at least

    int rangeX = !need_Xscroll ? 0 : thumbX + (virtSize.x - gw + thumbX + 1)/thumbX;
    int rangeY = !need_Yscroll ? 0 : thumbY + (virtSize.y - gh + thumbY + 1)/thumbY;

    int pageX = int((gw - thumbX) / thumbX);
    int pageY = int((gh - thumbY) / thumbY);

    //(x + GRID_SCROLL_LINE_X - 1) / GRID_SCROLL_LINE_X
    int posX = m_gridOrigin.x / thumbX; //Huh? why not divide by thumb in MSW?
    int posY = m_gridOrigin.y / thumbY; //

#endif
    //wxPrintf(wxT("Pos %d %d, virtSize %d %d, range %d %d, thumb %d %d page %d %d, win %d %d \n"),
    //    posX, posY, virtSize.x, virtSize.y, rangeX, rangeY, thumbX, thumbY, pageX, pageY, gw, gh); fflush(stdout);
    ...
    //    m_horizScrollBar->GetThumbPosition(), m_horizScrollBar->GetRange(), m_horizScrollBar->GetThumbSize(), m_horizScrollBar->GetPageSize()); fflush(stdout);

    // recalculate the windows sizes and positions if we added/removed scrollbar
    bool recalc_sizes = false;
    ...
    if (recalc_sizes && calc_win_sizes)
        CalcWindowSizes(false);

    // readjust, if necessary, the equal col widths if hiding or showing SB
    if (need_Yscroll != vertSbShown)
...
void wxSheet::ProcessSheetMouseEvent( wxMouseEvent& event )
{
    ...
    if (event.LeftDown())
    {
        if (wxRECT_CONTAINS(m_vertSplitRect, mousePos) || wxRECT_CONTAINS(m_horizSplitRect, mousePos))
    ...
    else if (event.Dragging() && HasCapture() && HasMouseCursorMode(WXSHEET_CURSOR_SPLITTING))
    {
        ...
        if (HasMouseCursorMode(WXSHEET_CURSOR_SPLIT_VERTICAL))
            splitEvent.m_split_mode = wxSHEET_SPLIT_VERTICAL;
        else
            splitEvent.m_split_mode = wxSHEET_SPLIT_HORIZONTAL;
    ...
    {
        if (wxRECT_CONTAINS(m_vertSplitRect, mousePos))
            SetMouseCursorMode(WXSHEET_CURSOR_SPLIT_VERTICAL, this);
        else if (wxRECT_CONTAINS(m_horizSplitRect, mousePos))
...
void wxSheet::ProcessRowLabelMouseEvent( wxMouseEvent& event )
{
    ...
    if ( event.Dragging() && event.LeftIsDown() )
    {
        ...
        if ( event.LeftIsDown() )
        {
            ...
            switch ( GetMouseCursorMode() )
            {
                ...
                {
                    int row_top    = GetRowTop(m_dragRowOrCol);
                    int min_height = GetMinimalRowHeight(m_dragRowOrCol);
                    y = wxMax( y, row_top + min_height );
                    can_scroll = false; // y != m_dragLastPos;

                    if (SendCellSizeEvent(wxEVT_SHEET_ROW_SIZING, wxSheetCoords(m_dragRowOrCol, -1),
                                          y-row_top, &event) != EVT_VETOED)
                    {
                ...
                {
                    if (HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectCols|wxSHEET_SelectSingle))
                        break;
    ...
    if ( event.LeftDown() )
    {
        ...
        if ( m_dragRowOrCol >= 0 )
        {
            ...
            if ( CanDragRowSize()  &&
                 (SendCellSizeEvent(wxEVT_SHEET_ROW_SIZING, wxSheetCoords(m_dragRowOrCol, -1),
                                    GetRowHeight(m_dragRowOrCol), &event) != EVT_VETOED) )
            {
                SetMouseCursorMode(WXSHEET_CURSOR_RESIZE_ROW, m_rowLabelWin);
                SetCaptureWindow(m_rowLabelWin);
            }
            else
                m_dragRowOrCol = -1; // maybe they want to select cells instead
        }

        // don't send a label click event for a hit on the edge of the row label
        // this is probably the user wanting to resize the row
        if ( m_dragRowOrCol < 0 ) //YToEdgeOfGridRow(y) < 0 )
        {
            if ( ContainsRowLabelCell(coords)  &&
                 (SendEvent(wxEVT_SHEET_LABEL_LEFT_DOWN, coords, &event) == EVT_SKIPPED) &&
                 !HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectCols|wxSHEET_SelectSingle) )
            {
                bool add = event.ShiftDown() || event.ControlDown();
                wxSheetBlock block;
                ...
                if (!add && HasSelection() &&
                    (SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTING,
        ...
        if ( row < 0 )
        {
                 ...
                 (SendEvent(wxEVT_SHEET_LABEL_LEFT_DCLICK, coords, &event) == EVT_SKIPPED))
            {
                if (CanEnableCellControl(coords))
                {
                    ...
                    if ( IsCellEditControlCreated() )
                        GetSheetRefData()->m_cellEditor.StartingClick(event);
                }
            }
        }
        else
    ...
    else if ( event.LeftUp() )
    {
        ...
        if ( HasMouseCursorMode(WXSHEET_CURSOR_RESIZE_ROW) )
        {
            ...
            if (DoEndDragResizeRowCol())
                SendCellSizeEvent( wxEVT_SHEET_ROW_SIZED, wxSheetCoords(m_dragRowOrCol, -1),
                                   GetRowHeight(m_dragRowOrCol), &event );
        }
        else if ( HasMouseCursorMode(WXSHEET_CURSOR_SELECT_ROW) )
        {
            if (!GetSelectingBlock().IsEmpty())
            {
                ...
                if (SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTING,
    ...
    else if ( event.Moving() )
    {
        ...
        if ( m_dragRowOrCol >= 0 )
        {
            if ( HasMouseCursorMode(WXSHEET_CURSOR_SELECT_CELL) )
            {
                ...
                if ( CanDragRowSize()  &&
                    (SendCellSizeEvent(wxEVT_SHEET_ROW_SIZE, wxSheetCoords(m_dragRowOrCol, -1),
                                       GetRowHeight(m_dragRowOrCol), &event) != EVT_VETOED) )
                {
                    SetMouseCursorMode(WXSHEET_CURSOR_RESIZE_ROW, m_rowLabelWin);
                }
...
void wxSheet::ProcessColLabelMouseEvent( wxMouseEvent& event )
{
    ...
    if ( event.Dragging() && event.LeftIsDown() )
    {
        ...
        if ( event.LeftIsDown() )
        {
            ...
            switch( GetMouseCursorMode() )
            {
                ...
                {
                    int col_left  = GetColLeft(m_dragRowOrCol);
                    int min_width = GetMinimalColWidth(m_dragRowOrCol);
                    x = wxMax( x, col_left + min_width );
                    can_scroll = false; // x != m_dragLastPos;
                    if (SendCellSizeEvent(wxEVT_SHEET_COL_SIZING, wxSheetCoords(-1, m_dragRowOrCol),
                                          x-col_left, &event) != EVT_VETOED)
                    {
                ...
                {
                    if (HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectRows|wxSHEET_SelectSingle))
                        break;
    ...
    if ( event.LeftDown() )
    {
        ...
        if ( m_dragRowOrCol >= 0 )
        {
            ...
            if ( CanDragColSize() &&
                (SendCellSizeEvent(wxEVT_SHEET_COL_SIZING, wxSheetCoords(-1, m_dragRowOrCol),
                                   GetColWidth(m_dragRowOrCol), &event) != EVT_VETOED) )
            {
        ...
        if ( m_dragRowOrCol < 0 ) //XToEdgeOfGridCol(x) < 0 )
        {
            if ( ContainsColLabelCell(coords) &&
                 (SendEvent(wxEVT_SHEET_LABEL_LEFT_DOWN, coords, &event) == EVT_SKIPPED) &&
                 !HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectRows|wxSHEET_SelectSingle) )
            {
                bool add = event.ShiftDown() || event.ControlDown();
                wxSheetBlock block;
    ...
    else if ( event.LeftUp() )
    {
        ...
        if ( HasMouseCursorMode(WXSHEET_CURSOR_RESIZE_COL) )
        {
            // Note: send event *after* doing default processing in this case
            if (DoEndDragResizeRowCol())
                SendCellSizeEvent( wxEVT_SHEET_COL_SIZED, wxSheetCoords(-1, m_dragRowOrCol),
                                   GetColWidth(m_dragRowOrCol), &event );
        }
    ...
    else if ( event.Moving() )
    {
        ...
        if ( m_dragRowOrCol >= 0 )
        {
            if ( HasMouseCursorMode(WXSHEET_CURSOR_SELECT_CELL) )
            {
                // don't capture the mouse yet
                if ( CanDragColSize() &&
                    (SendCellSizeEvent(wxEVT_SHEET_COL_SIZE, wxSheetCoords(-1, m_dragRowOrCol),
                                       GetColWidth(m_dragRowOrCol), &event) != EVT_VETOED) )
                {
                    SetMouseCursorMode(WXSHEET_CURSOR_RESIZE_COL, m_colLabelWin);
                }
...
void wxSheet::ProcessCornerLabelMouseEvent( wxMouseEvent& event )
{
    ...
    if ( event.Dragging() )
    {
        if ( event.LeftIsDown() )
        {
            if ((GetNumberRows() > 0) && (GetNumberCols() > 0) &&
                !HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectSingle))
            {
    ...
    if ( event.LeftDown() )
    {
        ...
        if ( SendEvent(wxEVT_SHEET_LABEL_LEFT_DOWN, coords, &event) == EVT_SKIPPED)
        {
            ...
            bool can_edit = CanEnableCellControl(coords);
            if (!can_edit && !HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectSingle) &&
                (SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTING,
...
void wxSheet::ProcessGridCellMouseEvent( wxMouseEvent& event )
{
    ...
    if ( event.Dragging() && event.LeftIsDown() )
    {
        ...
        if ( HasMouseCursorMode(WXSHEET_CURSOR_SELECT_CELL) )
        {
            ...
            if ( ContainsGridCell(GetSelectingAnchor()) && ContainsGridCell(coords) &&
                 !HasSelectionMode(wxSHEET_SelectNone) )
            {
                if (HasSelectionMode(wxSHEET_SelectSingle))
                    HighlightSelectingBlock( GetSelectingAnchor(), GetSelectingAnchor());
                else
        ...
        if (can_scroll &&
            !wxRECT_CONTAINS(wxRect(wxPoint(0,0), m_gridWin->GetClientSize()), m_mousePos))
        {
            if (!m_mouseTimer)
                StartMouseTimer();
        }
    ...
    if ( event.LeftDown() && ContainsGridCell(coords) )
    {
        if ( SendEvent(wxEVT_SHEET_CELL_LEFT_DOWN, coords, &event) == EVT_SKIPPED)
        {
            ...
            {
                wxSheetBlock block(0, 0, GetNumberRows(), GetNumberCols());
            ...
            if ( event.ShiftDown() )
            {
                ...
                block = FixSelectionBlock(block);

                if (!HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectSingle) &&
                    SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTING,
            ...
            else if ( m_dragRowOrCol < 0 ) //(XToEdgeOfGridCol(x) < 0) && (YToEdgeOfGridRow(y) < 0) )
            {
                if (IsCellEditControlCreated())
                    DisableCellEditControl(true);
                ...
                if ( event.ControlDown() )
                {
                    ...
                    {
                        if (HasSelectionMode(wxSHEET_SelectRows) ||
                            HasSelectionMode(wxSHEET_SelectCols))
                        {
                            if (sel)
                                DeselectBlock(block, true);
                            else
                                SelectBlock(block, true, true);
                        }
                        else if (HasSelectionMode(wxSHEET_SelectSingle))
                            SelectCell(coords, false, true);
                        else
                ...
                else
                {
                    ...
                    SetGridCursorCell( coords );
                    // FIXME weird? Always highlight a whole row/col when not in select cells?
                    // wxGrid does this, I'm not sure it makes sense?
                    if ( !HasSelectionMode(wxSHEET_SelectNone|wxSHEET_SelectCells|wxSHEET_SelectSingle) &&
                          SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTING,
                                               block, true, false, &event) != EVT_VETOED)
                        HighlightSelectingBlock( block );
                }
    ...
    else if ( event.Moving() && !event.IsButton() )
    {
        ...
        if ( (m_dragRowOrCol < 0) && !HasMouseCursorMode(WXSHEET_CURSOR_SELECT_CELL) )
            SetMouseCursorMode(WXSHEET_CURSOR_SELECT_CELL, m_gridWin);
    }
}
        ...
        switch ( event.GetKeyCode() )
        {
            ...
            case WXK_SPACE:
            {
                if ( event.ShiftDown() && ContainsGridCell(GetGridCursorCell()) )
                {
                    ...
                    // Select whole row for shift+space
                    if (!HasSelectionMode(wxSHEET_SelectNone) &&
                        SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTING,
                                       block, true, false, &event) != EVT_VETOED)
                    {
                ...
                if ( event.ControlDown() && ContainsGridCell(GetGridCursorCell()) )
                {
                    ...
                    // Select whole col for shift+space
                    if (!HasSelectionMode(wxSHEET_SelectNone) &&
                        SendRangeSelectEvent(wxEVT_SHEET_RANGE_SELECTING,
                                       block, true, false, &event) != EVT_VETOED)
                    {
...
void wxSheet::OnChar( wxKeyEvent& event )
{
    ...
    switch ( event.GetKeyCode() )
    {
        case WXK_UP:
        {
            ...
            {
                int new_height = GetRowHeight(GetGridCursorRow()) - 5;
                if (SendCellSizeEvent(wxEVT_SHEET_ROW_SIZING, wxSheetCoords(GetGridCursorRow(), -1),
                                      new_height, &event) != EVT_VETOED)
                {
                    SetRowHeight(GetGridCursorRow(), new_height);
                    SendCellSizeEvent( wxEVT_SHEET_ROW_SIZED, wxSheetCoords(GetGridCursorRow(), -1),
                                       GetRowHeight(GetGridCursorRow()), &event );
                }
            }
            else
        ...
        case WXK_LEFT:
        {
            ...
            {
                int new_width = GetColWidth(GetGridCursorCol()) - 5;
                if (SendCellSizeEvent(wxEVT_SHEET_COL_SIZING, wxSheetCoords(-1, GetGridCursorCol()),
                                      new_width, &event) != EVT_VETOED)
                {
                    SetColWidth(GetGridCursorCol(), new_width);
                    SendCellSizeEvent( wxEVT_SHEET_COL_SIZED, wxSheetCoords(-1, GetGridCursorCol()),
                                       GetColWidth(GetGridCursorCol()), &event );
                }
            }
            else
        ...
        default :
        {
            ...
            if ( !IsCellEditControlCreated() && CanEnableCellControl(GetGridCursorCell()) )
            {
                     ...
                     || editor.IsAcceptedKey(event) )
                {
                    ...
                    // crash the app
                    if ((event.GetKeyCode() != WXK_F2) && IsCellEditControlCreated())
...
void wxSheet::OnMouseTimer( wxTimerEvent &WXUNUSED(event) )
{
    ...
    double rate = 40.0; // every "rate" pixels from win edge, double speed

    wxSize clientSize(win->GetClientSize());
    double dx = (m_mousePos.x < 0) ? -rate + m_mousePos.x :
                ((m_mousePos.x > clientSize.x) ? rate + m_mousePos.x - clientSize.x : 0);
    double dy = (m_mousePos.y < 0) ? -rate + m_mousePos.y :
                ((m_mousePos.y > clientSize.y) ? rate + m_mousePos.y - clientSize.y : 0);
    ...
    SetGridOrigin( m_gridOrigin.x + int(dx*GetSheetRefData()->m_scrollThumbSize.x/rate),
                   m_gridOrigin.y + int(dy*GetSheetRefData()->m_scrollThumbSize.y/rate),
                   true, true );
...
int wxSheet::SendEvent( const wxEventType type, const wxSheetCoords& coords,
                        ...
                        const wxString& cmdString, int cmd_int )
{
    ...
    sheetEvt.SetInt(cmd_int);
    sheetEvt.SetString(cmdString);
    sheetEvt.SetKeysDownMousePos(mouseOrKeyEvt);
    return DoSendEvent(&sheetEvt);
}

int wxSheet::SendCellSizeEvent( const wxEventType type, const wxSheetCoords& coords,
                                int new_size, wxEvent* mouseOrKeyEvt )
{
    wxCHECK_MSG((type == wxEVT_SHEET_ROW_SIZE)||(type == wxEVT_SHEET_ROW_SIZING)||(type == wxEVT_SHEET_ROW_SIZED)||
                (type == wxEVT_SHEET_COL_SIZE)||(type == wxEVT_SHEET_COL_SIZING)||(type == wxEVT_SHEET_COL_SIZED),
                0, wxT("Invalid event type in wxSheet::SendCellSizeEvent"));

    wxSheetCellSizeEvent sheetEvt(GetId(), type, this, coords, new_size);
    sheetEvt.SetKeysDownMousePos(mouseOrKeyEvt);
    return DoSendEvent(&sheetEvt);
}

int wxSheet::SendRangeSelectEvent( const wxEventType type, const wxSheetBlock& block,
                                   bool selecting, bool add, wxEvent *mouseOrKeyEvt )
{
    wxCHECK_MSG((type == wxEVT_SHEET_RANGE_SELECTED) || (type == wxEVT_SHEET_RANGE_SELECTING), 0,
                wxT("Invalid event type"));
...
int wxSheet::SendEditorCreatedEvent( const wxEventType type, const wxSheetCoords& coords,
                                     wxWindow* ctrl )
{

    wxSheetEditorCreatedEvent sheetEvt( GetId(), type, this, coords, ctrl );
    return GetEventHandler()->ProcessEvent(sheetEvt) ? EVT_CLAIMED : EVT_SKIPPED;
}
...
// don't let the rect have a size smaller than min_size
wxRect SheetFixWinRect(const wxRect& r, int min_size = 0)
{
    return wxRect(r.x, r.y, wxMax(min_size, r.width), wxMax(min_size, r.height));
}
...
void wxSheet::CalcWindowSizes(bool adjustScrollBars)
{
    ...
    bool horiz_splitter = vert_sb  && m_enable_split_horiz;
    bool vert_splitter  = horiz_sb && m_enable_split_vert;
    ...
    if ( horiz_sb )
    {
        ...
        if (vert_splitter)
        {
            ...
            m_vertSplitRect = wxRect(rect.GetRight(), rect.GetTop(), SPLIT_BUTTON_WIDTH, rect.GetHeight());
        }
        else
            m_vertSplitRect = wxRect(0,0,0,0);

        rect = SheetFixWinRect(rect);
    ...
    if ( vert_sb )
    {
        ...
        if (horiz_splitter)
        {
            ...
            m_horizSplitRect = wxRect(rect.GetLeft(), rect.GetTop(), rect.GetWidth(), SPLIT_BUTTON_WIDTH);
    ...
    if ( m_gridWin->IsShown() )
    {
        ...
        //PRINT_RECT("Get grid rect ", m_gridWin->GetRect());
    }
...
wxSize wxSheet::DoGetBestSize() const
{
    ...
    wxSize size(GetSheetVirtualSize(true));
