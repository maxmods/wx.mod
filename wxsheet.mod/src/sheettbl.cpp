...
*/
    ...
    const size_t chars = col == 0 ? 0 : size_t(log(double(col))/3.2580965380); // log_26(x) = log(x)/log(26.0)
...
{
    wxFAIL_MSG(wxString::Format(wxT("Unable to return wxSheetTable::GetValueAsCustom for cell (%d, %d) and type '%s'"),
...
void  wxSheetTable::SetValueAsCustom( const wxSheetCoords& coords,
                                          ...
                                          void* WXUNUSED(value) )
{
    wxFAIL_MSG(wxString::Format(wxT("Unable to set wxSheetTable::SetValueAsCustom for cell (%d, %d) and type '%s'"),
                      coords.m_row, coords.m_col, typeName.c_str()));
}
