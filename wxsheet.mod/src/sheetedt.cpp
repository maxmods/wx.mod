...
bool wxSheetCellEditor::DestroyControl()
{
    wxCHECK_MSG(Ok(), false, wxT("wxSheetCellEditor not created"));
    return M_CELLEDITORDATA->DestroyControl();
}
...
void wxSheetCellEditor::StartingClick(wxMouseEvent& event)
{
    wxCHECK_RET(Ok(), wxT("wxSheetCellEditor not created"));
    M_CELLEDITORDATA->StartingClick(event);
}
...
bool wxSheetCellEditorRefData::DestroyControl()
{
    if (m_control)
    {
...
bool wxSheetCellTextEditorRefData::OnChar( wxKeyEvent& WXUNUSED(event) )
{
    ...
    if (GetTextCtrl() == NULL) return true;
...
wxSize wxSheetCellNumberEditorRefData::GetBestSize(wxSheet& sheet, const wxSheetCellAttr& attr,
                                                   const wxSheetCoords& coords)
{
    static wxSize s_spinSize(-1, -1);

    if (GetTextCtrl() != NULL)
        return wxSheetCellTextEditorRefData::GetBestSize(sheet, attr, coords);
    ...
    if (s_spinSize == wxSize(-1, -1))
        s_spinSize = GetSpinCtrl()->GetBestSize();

    return wxSize(w + 4, s_spinSize.y);
}
...
void wxSheetCellBoolEditorRefData::StartingClick(wxMouseEvent& )
{
    wxCHECK_RET(GetControl(), wxT("The wxSheetCellEditor must be Created first!"));
